...
const Navbar: React.FC = () => {
    ...
    return (
        <header className="w-full py-3 px-24 h-fit text-slate-900 bg-white shadow-sm flex justify-between items-center fixed top-0 font-['Quicksand'] z-50">
            <div className="logo">
                <h1 className="text-xl font-semibold font-['Public Sans'] flex flex-row items-center gap-2"><i className="bi bi-controller block -rotate-12"></i>Gaming Portal</h1>
            </div>
            <nav className="w-fit">
                ...
                <ul className={`w-fit lg:w-full flex flex-col lg:flex-row gap-10 absolute right-5 border lg:border-none border-slate-300 rounded-md shadow-md lg:shadow-none py-5 px-10 lg:p-0 lg:static justify-center transition-all duration-300 ease-out bg-white items-center ${isActiveNav ? "opacity-100 top-12" : "top-8 pointer-events-none lg:pointer-events-auto opacity-0 lg:opacity-100"}`}>
                    {localStorage["portalToken"] ? (
                        <>
                            <li><Link className={`font-['Public Sans'] text-sm ${pathname !== "/" ? "opacity-50" : ""}`} href={'/'}>Home</Link></li>
                            <li><Link className={`font-['Public Sans'] text-sm ${activePath !== "discover" ? "opacity-50" : ""}`} href={'/discover'}>Discover Games</Link></li>
                            <li><Link className={`font-['Public Sans'] text-sm ${activePath !== "manage-games" ? "opacity-50" : ""}`} href={'/manage-games'}>Manage Games</Link></li>
                            <li><Link className={`font-['Public Sans'] text-sm ${activePath !== "profile" ? "opacity-50" : ""}`} href={'/profile'}>Profile</Link></li>
                            <li><Link className={`font-['Public Sans'] text-sm ${activePath !== "community" ? "opacity-50" : ""}`} href={'/community'}>Community</Link></li>
                            <li><Button type="button" className="rounded-full px-5 !text-sm font-normal flex items-center gap-2" handleClick={onLogout}><i className="bi bi-box-arrow-left"></i>Sign Out</Button></li>
                            {/* <li><Button type="button" className="rounded-full px-5 !text-sm font-normal flex items-center gap-2 bg-green-600 text-white" handleClick={onLogout}><i className="bi bi-plus"></i>Upload New</Button></li> */}
                        </>
                    ) : (
